...
class AvailabilityController extends FrontendController{
    ...
    protected $indexView = 'Accommodation::frontend.user.availability';
    ...
    public function index(Request $request){
        ...
        $q = $this->accommodationClass::query();
        ...
        $q->orderBy('bravo_accommodations.id','desc');
        ...
        $page_title = __('Accommodations Availability');
    ...
    public function loadDates(Request $request){
        ...
        $accommodation = $this->accommodationClass::find($request->query('id'));
        if(empty($accommodation)){
            return $this->sendError(__('Accommodation not found'));
        }
        $is_single = $request->query('for_single');

        $query = $this->accommodationDateClass::query();
...
//        {
        ...
        foreach ($period as $dt){
            ...
            $date = [
                'id'=>rand(0,999),
                'active'=>0,
                'price'=>(!empty($accommodation->sale_price) and $accommodation->sale_price > 0 and $accommodation->sale_price < $accommodation->price) ? $accommodation->sale_price : $accommodation->price,
                'is_instant'=>$accommodation->is_instant,
                'is_default'=>true,
                'textColor'=>'#2791fe'
            ];
            if(!$is_single){
            ...
            if($accommodation->default_state){
                $date['active'] = 1;
            }else{
        ...
        $bookings = $this->bookingClass::getBookingInRanges($accommodation->id,$accommodation->type,$request->query('start'),$request->query('end'));
        if(!empty($bookings))
        {
...
//                for($i = strtotime($booking->start_date); $i <= strtotime($booking->end_date); $i+= DAY_IN_SECONDS){
	    ...
	    if(!empty($accommodation->ical_import_url)){

		    $startDate = $request->query('start');
		    $endDate = $request->query('end');
		    $timezone = setting_item('site_timezone',config('app.timezone'));
		    try {
			    $icalevents   =  new Ical($accommodation->ical_import_url,[
    ...
    public function store(Request $request){
        ...
        $accommodation = $this->accommodationClass::find($request->input('target_id'));
        ...
        if(!$this->hasPermission('accommodation_manage_others')){
            if($accommodation->create_user != Auth::id()){
...
//        {
        ...
        foreach ($period as $dt){
            $date = $this->accommodationDateClass::where('start_date',$dt->format('Y-m-d'))->where('target_id',$target_id)->first();

            if(empty($date)){
                $date = new $this->accommodationDateClass();
