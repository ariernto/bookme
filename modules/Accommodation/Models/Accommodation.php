...
class Accommodation extends Bookable
{
    use SoftDeletes;
    protected $table = 'bravo_accommodations';
    public $type = 'accommodation';
    public $checkout_booking_detail_file       = 'Accommodation::frontend/booking/detail';
    public $checkout_booking_detail_modal_file = 'Accommodation::frontend/booking/detail-modal';
    public $email_new_booking_file             = 'Accommodation::emails.new_booking_detail';
    public $availabilityClass = AccommodationDate::class;
    ...
    protected $seo_type = 'accommodation';
    ...
    public function __construct(array $attributes = [])
    {
        ...
        $this->accommodationDateClass = AccommodationDate::class;
    ...
    {
        $meta['seo_title'] = __("Search for Accommodations");
        if (!empty($title = setting_item_with_lang("accommodation_page_list_seo_title",false))) {
            $meta['seo_title'] = $title;
        }else if(!empty($title = setting_item_with_lang("accommodation_page_search_title"))) {
            $meta['seo_title'] = $title;
        }
        $meta['seo_image'] = null;
        if (!empty($title = setting_item("accommodation_page_list_seo_image"))) {
            $meta['seo_image'] = $title;
        }else if(!empty($title = setting_item("accommodation_page_search_banner"))) {
            $meta['seo_image'] = $title;
        }
        $meta['seo_desc'] = setting_item_with_lang("accommodation_page_list_seo_desc");
        $meta['seo_share'] = setting_item_with_lang("accommodation_page_list_seo_share");
        $meta['full_url'] = url(config('accommodation.accommodation_route_prefix'));
        return $meta;
    }


    public function terms(){
        return $this->hasMany($this->accommodationTermClass, "target_id");
    }
    ...
    public function getDetailUrl($include_param = true)
    {
        ...
        $urlDetail = app_get_locale(false, false, '/') . config('accommodation.accommodation_route_prefix') . "/" . $this->slug;
        if(!empty($param)){
    ...
    public static function getLinkForPageSearch( $locale = false , $param = [] ){
        ...
        return url(app_get_locale(false , false , '/'). config('accommodation.accommodation_route_prefix')."?".http_build_query($param));
    }
    ...
    {
        return url(route('accommodation.admin.edit',['id'=>$this->id]));
    }
    ...
    public function addToCart(Request $request)
    {
        ...
        $list_fees = setting_item('accommodation_booking_buyer_fees');
        if (!empty($list_fees)) {
            $total_fee = 0;
    ...
    public function getPriceInRanges($start_date,$end_date){
        ...
        $datesRaw = $this->accommodationDateClass::getDatesInRanges($start_date,$end_date,$this->id);
        $dates = [];
        if(!empty($datesRaw))
        {
...
//        for($i = strtotime($start_date); $i <= strtotime($end_date); $i += DAY_IN_SECONDS){
    ...
    public function addToCartValidate(Request $request)
    {
        ...
        if(!$this->isAvailableInRanges($start_date,$end_date)){
            return $this->sendError(__("This accommodation is not available at selected dates"));
        }
    ...
    public function isAvailableInRanges($start_date,$end_date){
        ...
        {
            $notAvailableDates = $this->accommodationDateClass::query()->where([
            ...
            $availableDates = $this->accommodationDateClass::query()->where([
                ['start_date','>=',$start_date],
                ['end_date','<=',$end_date],
                ['active','=',1],
                ['target_id','=',$this->id],
    ...
    public function getBookingData()
    {
        ...
        $list_fees = setting_item_array('accommodation_booking_buyer_fees');
        if(!empty($list_fees)){
    ...
    {
        return setting_item("accommodation_enable_review", 0);
    }

    public function getReviewApproved()
    {
        return setting_item("accommodation_review_approved", 0);
    }

    public function check_enable_review_after_booking()
    {
        $option = setting_item("accommodation_enable_review_after_booking", 0);
        if ($option) {
    ...
    {
        $options = setting_item("accommodation_allow_review_after_making_completed_booking", false);
        if (!empty($options)) {
    ...
    {
        $reviewStats = [];
        if (!empty($list = setting_item("accommodation_review_stats", []))) {
     ...
     * Get Score Review
     *
     * Using for loop accommodation
     */
    public function getScoreReview()
    {
        $accommodation_id = $this->id;
        $list_score = Cache::rememberForever('review_'.$this->type.'_' . $accommodation_id, function () use ($accommodation_id) {
            $dataReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id', $accommodation_id)->where('object_model', "accommodation")->where("status", "approved")->first();
    ...
    public function getReviewList(){
        return $this->reviewClass::select(['id','title','content','rate_number','author_ip','status','created_at','vendor_id','create_user'])->where('object_id', $this->id)->where('object_model', 'accommodation')->where("status", "approved")->orderBy("id", "desc")->with('author')->paginate(setting_item('accommodation_review_number_per_page', 5));
    }
    ...
    public function getNumberServiceInLocation($location)
    {
        ...
        if ($number > 1) {
            return __(":number Accommodations", ['number' => $number]);
        }
        return __(":number Accommodation", ['number' => $number]);
    }
    ...
    public function saveCloneByID($clone_id){
        ...
        //Language
        $langs = $this->accommodationTranslationClass::where("origin_id",$old->id)->get();
        if(!empty($langs)){
    ...
    public static function isEnable(){
        return setting_item('accommodation_disable') == false;
    }

    public function isDepositEnable(){
        return (setting_item('accommodation_deposit_enable') and setting_item('accommodation_deposit_amount'));
    }
    public function getDepositAmount(){
        return setting_item('accommodation_deposit_amount');
    }
    public function getDepositType(){
        return setting_item('accommodation_deposit_type');
    }
    public function getDepositFomular(){
        return setting_item('accommodation_deposit_fomular','default');
    }
...
//		    {
    ...
    public static function isEnableEnquiry(){
        if(!empty(setting_item('booking_enquiry_for_accommodation'))){
            return true;
        }
        return false;
    }
    ...
    public static function isFormEnquiryAndBook(){
        $check = setting_item('booking_enquiry_for_accommodation');
    ...
    {
        $model_accommodation = parent::query()->select("bravo_accommodations.*");
        $model_accommodation->where("bravo_accommodations.status", "publish");
        if (!empty($location_id = $request->query('location_id'))) {
            $location = Location::query()->where('id', $location_id)->where("status","publish")->first();
            if(!empty($location)){
                $model_accommodation->join('bravo_locations', function ($join) use ($location) {
                    $join->on('bravo_locations.id', '=', 'bravo_accommodations.location_id')
        ...
        if (!empty($price_range = $request->query('price_range'))) {
            ...
            $raw_sql_min_max = "( (IFNULL(bravo_accommodations.sale_price,0) > 0 and bravo_accommodations.sale_price >= ? ) OR (IFNULL(bravo_accommodations.sale_price,0) <= 0 and bravo_accommodations.price >= ? ) ) 
                            AND ( (IFNULL(bravo_accommodations.sale_price,0) > 0 and bravo_accommodations.sale_price <= ? ) OR (IFNULL(bravo_accommodations.sale_price,0) <= 0 and bravo_accommodations.price <= ? ) )";
            $model_accommodation->WhereRaw($raw_sql_min_max,[$pri_from,$pri_from,$pri_to,$pri_to]);
        }
        ...
        if (is_array($terms) && !empty($terms)) {
            ...
            if(!empty($terms)){
                $model_accommodation->join('bravo_accommodation_term as tt', 'tt.target_id', "bravo_accommodations.id")->whereIn('tt.term_id', $terms);
        ...
        if (is_array($review_scores) && !empty($review_scores)) {
            ...
            foreach ($review_scores as $number){
                $where_review_score[] = " ( bravo_accommodations.review_score >= {$number} AND bravo_accommodations.review_score <= {$number}.9 ) ";
            }
            $sql_where_review_score = " ( " . implode("OR", $where_review_score) . " )  ";
            $model_accommodation->WhereRaw($sql_where_review_score);
        }

        if(!empty( $service_name = $request->query("service_name") )){
            if( setting_item('site_enable_multi_lang') && setting_item('site_locale') != app()->getLocale() ){
                $model_accommodation->leftJoin('bravo_accommodation_translations', function ($join) {
                    $join->on('bravo_accommodations.id', '=', 'bravo_accommodation_translations.origin_id');
                });
                $model_accommodation->where('bravo_accommodation_translations.title', 'LIKE', '%' . $service_name . '%');

            }else{
                $model_accommodation->where('bravo_accommodations.title', 'LIKE', '%' . $service_name . '%');
            }
        }
        if(!empty($lat = $request->query('map_lat')) and !empty($lgn = $request->query('map_lgn'))){
//            ORDER BY (POW((lon-$lon),2) + POW((lat-$lat),2))";
            $model_accommodation->orderByRaw("POW((bravo_accommodations.map_lng-".$lgn."),2) + POW((bravo_accommodations.map_lat-".$lat."),2)");
        }
        $orderby = $request->input("orderby");
        switch ($orderby){
            case "price_low_high":
                $raw_sql = "CASE WHEN IFNULL( bravo_accommodations.sale_price, 0 ) > 0 THEN bravo_accommodations.sale_price ELSE bravo_accommodations.price END AS tmp_min_price";
                $model_accommodation->selectRaw($raw_sql);
                $model_accommodation->orderBy("tmp_min_price", "asc");
                break;
            case "price_high_low":
                $raw_sql = "CASE WHEN IFNULL( bravo_accommodations.sale_price, 0 ) > 0 THEN bravo_accommodations.sale_price ELSE bravo_accommodations.price END AS tmp_min_price";
                $model_accommodation->selectRaw($raw_sql);
                $model_accommodation->orderBy("tmp_min_price", "desc");
                break;
            case "rate_high_low":
                $model_accommodation->orderBy("review_score", "desc");
                break;
            default:
                $model_accommodation->orderBy("is_featured", "desc");
                $model_accommodation->orderBy("id", "desc");
        }

        $model_accommodation->groupBy("bravo_accommodations.id");

        $max_guests = (int)($request->query('adults') + $request->query('children'));
        if($max_guests){
            $model_accommodation->where('max_guests','>=',$max_guests);
        }

        if(!empty($request->query('limit'))){
            $limit = $request->query('limit');
        }else{
            $limit = !empty(setting_item("accommodation_page_limit_item"))? setting_item("accommodation_page_limit_item") : 9;
        }
        return $model_accommodation->with(['location','hasWishList','translations'])->paginate($limit);
    }
    ...
    public function dataForApi($forSingle = false){
        ...
        if($forSingle){
            ...
            $data['booking_fee'] = setting_item_array('accommodation_booking_buyer_fees');
    ...
    {
        return "\Modules\Accommodation\Controllers\AvailabilityController";
    }
    ...
    static public function getFiltersSearch()
    {
        ...
        return [
            ...
            [
                ...
                "field"    => "terms",
                "position" => "3",
                "data" => Attributes::getAllAttributesForApi("accommodation")
            ]
        ];
    }
}
