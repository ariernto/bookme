@php($location_search_style = setting_item('tour_location_search_style'))
		...
		<div class="g-map-place" >
            <input type="text" name="map_place" placeholder="{{__("Tour Type")}}"  value="{{request()->input('map_place')}}" class="form-control searchinput border-0">
		...
		@else
			...
			<div class="smart-search">
                ...
                z-index: 2;
                "></div>
				<input type="text" class="smart-search-location parent_text form-control searchinput" {{ ( empty(setting_item("tour_location_search_style")) or setting_item("tour_location_search_style") == "normal" ) ? "readonly" : ""  }} placeholder="{{__("Tour type")}}" value="{{ $location_name }}" data-onLoad="{{__("Loading...")}}"
