...
class Activity extends Bookable
{
    use SoftDeletes;
    protected $table                              = 'bravo_activities';
    public    $checkout_booking_detail_file       = 'Activity::frontend/booking/detail';
    public    $checkout_booking_detail_modal_file = 'Activity::frontend/booking/detail-modal';
    public    $email_new_booking_file             = 'Activity::emails.new_booking_detail';
    public    $type                               = 'activity';
    public    $availabilityClass                  = ActivityDate::class;
    ...
    {
        return $this->hasOne("Modules\Activity\Models\ActivityCategory", "id", 'category_id')->with(['translations']);
    }
    ...
    {
        $meta['seo_title'] = __("Search for Activities");
        if (!empty($title = setting_item_with_lang("activity_page_list_seo_title", false))) {
            $meta['seo_title'] = $title;
        } else if (!empty($title = setting_item_with_lang("activity_page_search_title"))) {
            $meta['seo_title'] = $title;
        }
        $meta['seo_image'] = null;
        if (!empty($title = setting_item("activity_page_list_seo_image"))) {
            $meta['seo_image'] = $title;
        } else if (!empty($title = setting_item("activity_page_search_banner"))) {
            $meta['seo_image'] = $title;
        }
        $meta['seo_desc'] = setting_item_with_lang("activity_page_list_seo_desc");
        $meta['seo_share'] = setting_item_with_lang("activity_page_list_seo_share");
        $meta['full_url'] = url(config('activity.activity_route_prefix'));
        return $meta;
    }
    ...
    {
        return $this->hasOne($this->activityMetaClass, "activity_id");
    }
    ...
    {
        return $this->hasMany($this->activityTermClass, "activity_id");
    }
    ...
    public function getDetailUrl($include_param = true)
    {
        ...
        $urlDetail = app_get_locale(false, false, '/') . config('activity.activity_route_prefix') . "/" . $this->slug;
        if (!empty($param)) {
    ...
    public static function getLinkForPageSearch($locale = false, $param = [])
    {
        ...
        return url(app_get_locale(false, false, '/') . config('activity.activity_route_prefix') . "?" . http_build_query($param));
    }
    ...
    {
        return url('admin/module/activity/edit/' . $this->id);
    }
    ...
    {
        $meta = $this->activityMetaClass::where('activity_id', $this->id)->first();
        if (!$meta) {
            $meta = new $this->activityMetaClass();
            $meta->activity_id = $this->id;
        }
        $arg = $request->input();
    ...
    public function addToCart(Request $request)
    {
        ...
        $list_fees = setting_item('activity_booking_buyer_fees');
        $total_fee = 0;
        if (!empty($list_fees)) {
    ...
    {
        $datesRaw = $this->activityDateClass::getDatesInRanges($start_date, $this->id);
        $dates = [
            'base_price'   => null,
            'person_types' => null,
        ];
    ...
    {
        $activityDate = $this->activityDateClass::where('target_id', $this->id)->where('start_date', $start_date)->where('active', 1)->first();
        ...
        $maxGuests = !empty($activityDate->max_guests) ? $activityDate->max_guests : $this->max_people;
        ...
        if ($meta) {
            ...
            if ($meta->enable_open_hours) {
                ...
                if (!isset($open_hours[$nDate]) or empty($open_hours[$nDate]['enable'])) {
                    return $this->sendError(__("This activity is not open on your selected day"));
                }
            }
        }
    ...
    public function getBookingData()
    {
        ...
        $list_fees = setting_item_array('activity_booking_buyer_fees');
        if (!empty($list_fees)) {
    ...
    {
        return setting_item("activity_enable_review", 0);
    }

    public function getReviewApproved()
    {
        return setting_item("activity_review_approved", 0);
    }

    public function check_enable_review_after_booking()
    {
        $option = setting_item("activity_enable_review_after_booking", 0);
        if ($option) {
    ...
    {
        $options = setting_item("activity_allow_review_after_making_completed_booking", false);
        if (!empty($options)) {
    ...
    {
        $reviewStats = [];
        if (!empty($list = setting_item("activity_review_stats", []))) {
    ...
    public function getReviewDataAttribute()
    {
        ...
        $dataTotalReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id', $this->id)->where('object_model', "activity")->where("status", "approved")->first();
     ...
     * Get Score Review
     *
     * Using for loop activity
     */
    public function getScoreReview()
    {
        $activity_id = $this->id;
        $list_score = Cache::rememberForever('review_' . $this->type . '_' . $activity_id, function () use ($activity_id) {
            $dataReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id', $activity_id)->where('object_model', "activity")->where("status", "approved")->first();
            return [
    ...
    public function getNumberServiceInLocation($location)
    {
        ...
        if (!empty($location)) {
            $number = parent::join('bravo_locations', function ($join) use ($location) {
                $join->on('bravo_locations.id', '=', 'bravo_activities.location_id')->where('bravo_locations._lft', '>=', $location->_lft)->where('bravo_locations._rgt', '<=', $location->_rgt);
            })->where("bravo_activities.status", "publish")->with(['translations'])->count("bravo_activities.id");
        }
        if (empty($number))
            return false;
        if ($number > 1) {
            return __(":number Activities", ['number' => $number]);
        }
        return __(":number Activity", ['number' => $number]);
    }
    ...
    public function getReviewList(){
        return $this->reviewClass::select(['id','title','content','rate_number','author_ip','status','created_at','vendor_id','create_user'])
            ...
            ->orderBy("id", "desc")
            ->with('author')
            ->paginate(setting_item('activity_review_number_per_page', 5));
    }
    ...
    public function getBookingsInRange($from, $to)
    {
        ...
        $query->where('object_model', 'activity');
    ...
    public function saveCloneByID($clone_id)
    {
        ...
        if (empty($old))
            return false;
        $selected_terms = $old->activity_term->pluck('term_id');
        ...
        //Language
        $langs = $this->activityTranslationClass::where("origin_id", $old->id)->get();
        if (!empty($langs)) {
        ...
        //Meta
        $metaActivity = $this->activityMetaClass::where('activity_id', $old->id)->first();
        if (!empty($metaActivity)) {
            $metaActivityNew = $metaActivity->replicate();
            $metaActivityNew->activity_id = $new->id;
            $metaActivityNew->save();
        }
    }
    ...
    {
        return setting_item('activity_disable') == false;
    }

    public function isDepositEnable()
    {
        return (setting_item('activity_deposit_enable') and setting_item('activity_deposit_amount'));
    }

    public function getDepositAmount()
    {
        return setting_item('activity_deposit_amount');
    }

    public function getDepositType()
    {
        return setting_item('activity_deposit_type');
    }

    public function getDepositFomular()
    {
        return setting_item('activity_deposit_fomular', 'default');
    }

    public static function isEnableEnquiry()
    {
        if (!empty(setting_item('booking_enquiry_for_activity'))) {
            return true;
        }
        return false;
    }
    ...
    {
        $check = setting_item('booking_enquiry_for_activity');
    ...
    {
        $model_Activity = parent::query()->select("bravo_activities.*");
        ...
        if (!empty($price_range = $request->query('price_range'))) {
            ...
            $raw_sql_min_max = "( (IFNULL(bravo_activities.sale_price,0) > 0 and bravo_activities.sale_price >= ? ) OR (IFNULL(bravo_activities.sale_price,0) <= 0 and bravo_activities.price >= ?) ) 
								AND ( (IFNULL(bravo_activities.sale_price,0) > 0 and bravo_activities.sale_price <= ? ) OR (IFNULL(bravo_activities.sale_price,0) <= 0 and bravo_activities.price <= ?) )";
            $model_Activity->WhereRaw($raw_sql_min_max, [
                $pri_from,
                $pri_from,
                $pri_to,
                $pri_to
        ...
        if (!empty($category_ids = $request->query('cat_id'))) {
            ...
            if (!empty($list_cat)) {
                ...
                $model_Activity->join('bravo_activity_category', function ($join) use ($sql_where_join) {
                        $join->on('bravo_activity_category.id', '=', 'bravo_activities.category_id')->WhereRaw($sql_where_join);
                    });
            }
        }
        $terms = $request->query('terms');
        if (is_array($terms) && !empty($terms)) {
            $model_Activity->join('bravo_activity_term as tt', 'tt.activity_id', "bravo_activities.id")->whereIn('tt.term_id', $terms);
        }
        $review_scores = $request->query('review_score');
        if (is_array($review_scores) && !empty($review_scores)) {
            $where_review_score = [];
            foreach ($review_scores as $number) {
                $where_review_score[] = " ( bravo_activities.review_score >= {$number} AND bravo_activities.review_score <= {$number}.9 ) ";
            }
        ...
        if (!empty($service_name = $request->query("service_name"))) {
            if (setting_item('site_enable_multi_lang') && setting_item('site_locale') != app()->getLocale()) {
                $model_Activity->leftJoin('bravo_activity_translations', function ($join) {
                    $join->on('bravo_activities.id', '=', 'bravo_activity_translations.origin_id');
                });
                $model_Activity->where('bravo_activity_translations.title', 'LIKE', '%' . $service_name . '%');
            } else {
                $model_Activity->where('bravo_activities.title', 'LIKE', '%' . $service_name . '%');
            }
        }
        if(!empty($lat = $request->query('map_lat')) and !empty($lgn = $request->query('map_lgn'))){
//            ORDER BY (POW((lon-$lon),2) + POW((lat-$lat),2))";
            $model_Activity->orderByRaw("POW((bravo_activities.map_lng-".$lgn."),2) + POW((bravo_activities.map_lat-".$lat."),2)");
        }
        $orderby = $request->input("orderby");
        switch ($orderby){
            case "price_low_high":
                $raw_sql = "CASE WHEN IFNULL( bravo_activities.sale_price, 0 ) > 0 THEN bravo_activities.sale_price ELSE bravo_activities.price END AS tmp_min_price";
                $model_Activity->selectRaw($raw_sql);
                $model_Activity->orderBy("tmp_min_price", "asc");
                break;
            case "price_high_low":
                $raw_sql = "CASE WHEN IFNULL( bravo_activities.sale_price, 0 ) > 0 THEN bravo_activities.sale_price ELSE bravo_activities.price END AS tmp_min_price";
                $model_Activity->selectRaw($raw_sql);
                $model_Activity->orderBy("tmp_min_price", "desc");
                break;
            case "rate_high_low":
                $model_Activity->orderBy("review_score", "desc");
                break;
            default:
                $model_Activity->orderBy("is_featured", "desc");
        ...
        if(!empty($request->query('limit'))){
            ...
            $limit = !empty(setting_item("activity_page_limit_item"))? setting_item("activity_page_limit_item") : 9;
        }
        return $model_Activity->with([
            'location',
            'hasWishList',
            'translations'
        ])->paginate($limit);
    ...
    public function dataForApi($forSingle = false)
    {
        ...
        if ($forSingle) {
            ...
            $data['category'] = ActivityCategory::selectRaw("id,name")->find($this->category_id) ?? null;
            ...
            $data['booking_fee'] = setting_item_array('activity_booking_buyer_fees');
            ...
            $data['terms'] = Terms::getTermsByIdForAPI($this->activity_term->pluck('term_id'));
        } else {
    ...
    {
        return "\Modules\Activity\Controllers\AvailabilityController";
    }
    ...
    static public function getFiltersSearch()
    {
        ...
        $category = ActivityCategory::selectRaw("id,name,slug")->where('status', 'publish')->with(['translations'])->get()->toTree();
        return [
            [
            ...
            [
                ...
                "field"    => "terms",
                "position" => "4",
                "data" => Attributes::getAllAttributesForApi("activity")
            ]
        ];
    }
}
