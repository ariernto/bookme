<div class="g-header">
    ...
    <div class="right">
        @if(setting_item('activity_enable_review') and $review_score)
...
@if(!empty($row->duration) or !empty($row->category_activity->name) or !empty($row->max_people) or !empty($row->location->name))
    <div class="g-activity-feature">
    <div class="row">
        @if($row->duration)
        ...
        @endif
        @if(!empty($row->category_activity->name))
            @php $cat =  $row->category_activity->translateOrOrigin(app()->getLocale()) @endphp
            <div class="col-xs-6 col-lg-3 col-md-6">
                <div class="item">
                    <div class="icon">
                    ...
                    <div class="info">
                        <h4 class="name">{{__("Activity Type")}}</h4>
                        <p class="value">
                            {{$cat->name ?? ''}}
                        </p>
                    </div>
...
@endif
@include('Activity::frontend.layouts.details.activity-include-exclude')
@include('Activity::frontend.layouts.details.activity-itinerary')
@include('Activity::frontend.layouts.details.activity-attributes')
@include('Activity::frontend.layouts.details.activity-faqs')
@if($row->map_lat && $row->map_lng)
<div class="g-location">
    <div class="location-title">
        <h3>{{__("Activity Location")}}</h3>
        @if($translation->address)
            <div class="address">
