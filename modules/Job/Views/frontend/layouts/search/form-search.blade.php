            ...
            @if(!empty($hotel_search_fields))
                @foreach($hotel_search_fields as $field)
                    ...
                    <div class="col-md-{{ $field['size'] ?? "6" }} border-right">
                        @switch($field['field'])
                            @case ('service_name')
                                @include('Job::frontend.layouts.search.fields.service_name')
                            @break
                            @case ('location')
                                @include('Job::frontend.layouts.search.fields.location')
                            @break
                            @case ('date')
                                @include('Job::frontend.layouts.search.fields.date')
                            @break
                            @case ('guests')
                                @include('Job::frontend.layouts.search.fields.guests')
                            @break
                        @endswitch
                    </div>
                @endforeach
