    ...
    <div class="container-fluid">
        <div class="d-flex align-items-center mb40">
            ...
            <div class="title-actions ml-3">
                <a href="{{url('admin/module/news/create')}}" class="btn btn-sm"><small>{{__("+ ADD NEW")}}</small></a>
            </div>
        </div>
            ...
            <div class="col-left">
                @if(!empty($rows))
                    <form method="post" action="{{url('admin/module/news/bulkEdit')}}"
                        ...
                        <button data-confirm="{{__("Do you want to delete?")}}" class="btn-danger btn btn-icon rounded dungdt-apply-form-btn" type="button">{{__('APPLY')}}</button>
                    </form>
                @endif
            </div>
            <div class="col-left">
                <form method="get" action="{{url('/admin/module/news/')}} " class="filter-form filter-form-right d-flex justify-content-end flex-column flex-sm-row" role="search">
                    ...
                    <button class="btn-danger btnbtn-danger btn-icon rounded btn_search" type="submit">{{__('SEARCH')}}</button>
                </form>
            </div>
        </div>
                                ...
                                @if($rows->total() > 0)
                                    @foreach($rows as $row)
                                        <tr>
                                            ...
                                            <td>{{ $row->status }}</td>
                                            <td>
                                                <a href="{{route('news.admin.edit',['id'=>$row->id])}}" class="btn btn-sm"><i class="fa fa-edit"></i></a>
                                            </td>
                                        </tr>
                                    @endforeach
                                @else
