...
class Boat extends Bookable
{
    ...
    public function addToBoatt(Request $request)
    {
        $res = $this->addToBoattValidate($request);
        ...
        $list_fees = setting_item('boat_booking_buyer_fees');
        if (!empty($list_fees)) {
            $total_fee = 0;
    ...
    public function addToBoattValidate(Request $request)
    {
        $rules = [
            'number' => 'required',
...
//        }
    ...
    {
        $option = setting_item("boat_enable_review_after_booking", 0);
        if ($option) {
    ...
    public static function search(Request $request)
    {
        ...
        if (!empty($price_range = $request->query('price_range'))) {
            ...
            $raw_sql_min_max = "( (IFNULL(bravo_boats.sale_price,0) > 0 and bravo_boats.sale_price >= ? ) OR (IFNULL(bravo_boats.sale_price,0) <= 0 and bravo_boats.price >= ? ) ) 
                            AND ( (IFNULL(bravo_boats.sale_price,0) > 0 and bravo_boats.sale_price <= ? ) OR (IFNULL(bravo_boats.sale_price,0) <= 0 and bravo_boats.price <= ? ) )";
            $model_boat->WhereRaw($raw_sql_min_max,[$pri_from,$pri_from,$pri_to,$pri_to]);
        }
        ...
        if (is_array($review_scores) && !empty($review_scores)) {
            ...
            foreach ($review_scores as $number){
                $where_review_score[] = " ( bravo_boats.review_score >= {$number} AND bravo_boats.review_score <= {$number}.9 ) ";
            }
            $sql_where_review_score = " ( " . implode("OR", $where_review_score) . " )  ";
            $model_boat->WhereRaw($sql_where_review_score);
        }
...
//            ORDER BY (POW((lon-$lon),2) + POW((lat-$lat),2))";
        ...
        switch ($orderby){
            case "price_low_high":
                $raw_sql = "CASE WHEN IFNULL( bravo_boats.sale_price, 0 ) > 0 THEN bravo_boats.sale_price ELSE bravo_boats.price END AS tmp_min_price";
                $model_boat->selectRaw($raw_sql);
                $model_boat->orderBy("tmp_min_price", "asc");
                break;
            case "price_high_low":
                $raw_sql = "CASE WHEN IFNULL( bravo_boats.sale_price, 0 ) > 0 THEN bravo_boats.sale_price ELSE bravo_boats.price END AS tmp_min_price";
                $model_boat->selectRaw($raw_sql);
                $model_boat->orderBy("tmp_min_price", "desc");
                break;
            case "rate_high_low":
