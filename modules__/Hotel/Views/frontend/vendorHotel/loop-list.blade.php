    ...
    <div class="row">
        ...
        <div class="col-md-9">
            ...
            <div class="control-action">
                ...
                @if(!empty($recovery))
                    <a href="{{ route("hotel.vendor.restore",[$row->id]) }}" class="btn btn-recovery btn-light" data-confirm="{{__('"Do you want to recovery?"')}}">{{__("Recovery")}}</a>
                @endif
                @if(Auth::user()->hasPermissionTo('hotel_update'))
                    <a href="{{ route("hotel.vendor.edit",[$row->id]) }}" class="btn btn-light">{{__("Edit")}}</a>
                @endif
                @if(Auth::user()->hasPermissionTo('hotel_delete'))
                    <a href="{{ route("hotel.vendor.delete",[$row->id]) }}" class="btn btn-danger" data-confirm="{{__('"Do you want to delete?"')}}>">{{__("Delete")}}</a>
                @endif
                @if($row->status == 'publish')
                    <a href="{{ route("hotel.vendor.bulk_edit",[$row->id,'action' => "make-hide"]) }}" class="btn btn-light">{{__("Make hide")}}</a>
                @endif
                @if($row->status == 'draft')
