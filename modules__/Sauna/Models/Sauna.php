...
class Sauna extends Bookable
{
    use SoftDeletes;
    protected $table = 'bravo_saunas';
    public $type = 'sauna';
    public $checkout_booking_detail_file       = 'Sauna::frontend/booking/detail';
    public $checkout_booking_detail_modal_file = 'Sauna::frontend/booking/detail-modal';
    public $email_new_booking_file             = 'Sauna::emails.new_booking_detail';
    ...
    {
        $meta['seo_title'] = __("Search for Saunas");
        if (!empty($title = setting_item_with_lang("sauna_page_list_seo_title",false))) {
            $meta['seo_title'] = $title;
        }else if(!empty($title = setting_item_with_lang("sauna_page_search_title"))) {
            $meta['seo_title'] = $title;
        }
        $meta['seo_image'] = null;
        if (!empty($title = setting_item("sauna_page_list_seo_image"))) {
            $meta['seo_image'] = $title;
        }else if(!empty($title = setting_item("sauna_page_search_banner"))) {
            $meta['seo_image'] = $title;
        }
        $meta['seo_desc'] = setting_item_with_lang("sauna_page_list_seo_desc");
        $meta['seo_share'] = setting_item_with_lang("sauna_page_list_seo_share");
        $meta['full_url'] = url(env('SAUNA_ROUTE_PREFIX','sauna'));
        return $meta;
    }

    public function terms(){
        return $this->hasMany($this->saunaTermClass, "target_id");
    }
    ...
    public function getDetailUrl($include_param = true)
    {
        ...
        $urlDetail = app_get_locale(false, false, '/') . env('SAUNA_ROUTE_PREFIX','sauna') . "/" . $this->slug;
        if(!empty($param)){
    ...
    public static function getLinkForPageSearch( $locale = false , $param = [] ){
        ...
        return url(app_get_locale(false , false , '/'). env('SAUNA_ROUTE_PREFIX','sauna')."?".http_build_query($param));
    }
    ...
    {
        return url(route('sauna.admin.edit',['id'=>$this->id]));
    }
    ...
    public function addToCart(Request $request)
    {
        ...
        $list_fees = setting_item('sauna_booking_buyer_fees');
        $total_fee = 0;
        if (!empty($list_fees)) {
    ...
    public function getDataAvailableBooking($start_date)
    {
        ...
        $saunaDate = $this->saunaDateClass::where('target_id', $this->id)->where('start_date', $start_date)->where('active', 1)->first();
        if(!empty($saunaDate->ticket_types)){
            $ticket_types = $saunaDate->ticket_types;
        }
    ...
    public function getBookingData()
    {
        ...
        $list_fees = setting_item_array('sauna_booking_buyer_fees');
        if(!empty($list_fees)){
    ...
    {
        return setting_item("sauna_enable_review", 0);
    }

    public function getReviewApproved()
    {
        return setting_item("sauna_review_approved", 0);
    }

    public function check_enable_review_after_booking()
    {
        $option = setting_item("sauna_enable_review_after_booking", 0);
        if ($option) {
    ...
    {
        $options = setting_item("sauna_allow_review_after_making_completed_booking", false);
        if (!empty($options)) {
    ...
    {
        $reviewStats = [];
        if (!empty($list = setting_item("sauna_review_stats", []))) {
    ...
    {
        $sauna_id = $this->id;
        $list_score = Cache::rememberForever('review_'.$this->type.'_' . $sauna_id, function () use ($sauna_id) {
            $dataReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id', $sauna_id)->where('object_model', "sauna")->where("status", "approved")->first();
    ...
    public function getReviewList(){
        return $this->reviewClass::select(['id','title','content','rate_number','author_ip','status','created_at','vendor_id','create_user'])->where('object_id', $this->id)->where('object_model', 'sauna')->where("status", "approved")->orderBy("id", "desc")->with('author')->paginate(setting_item('sauna_review_number_per_page', 5));
    }
    ...
    public function getNumberServiceInLocation($location)
    {
        ...
        if ($number > 1) {
            return __(":number Saunas", ['number' => $number]);
        }
        return __(":number Sauna", ['number' => $number]);
    }
    ...
    public function saveCloneByID($clone_id){
        ...
        //Language
        $langs = $this->saunaTranslationClass::where("origin_id",$old->id)->get();
        if(!empty($langs)){
    ...
    public static function isEnable(){
        return setting_item('sauna_disable') == false;
    }
    ...
    public function isDepositEnable(){
        return (setting_item('sauna_deposit_enable') and setting_item('sauna_deposit_amount'));
    }
    public function getDepositAmount(){
        return setting_item('sauna_deposit_amount');
    }
    public function getDepositType(){
        return setting_item('sauna_deposit_type');
    }
    public function getDepositFomular(){
        return setting_item('sauna_deposit_fomular','default');
    }
    ...
    public static function isEnableEnquiry(){
        if(!empty(setting_item('booking_enquiry_for_sauna'))){
            return true;
        }
        return false;
    }
    public static function isFormEnquiryAndBook(){
        $check = setting_item('booking_enquiry_for_sauna');
    ...
    {
        $model_sauna = parent::query()->select("bravo_saunas.*");
        $model_sauna->where("bravo_saunas.status", "publish");
        if (!empty($location_id = $request->query('location_id'))) {
            $location = Location::where('id', $location_id)->where("status","publish")->first();
            if(!empty($location)){
                $model_sauna->join('bravo_locations', function ($join) use ($location) {
                    $join->on('bravo_locations.id', '=', 'bravo_saunas.location_id')
        ...
        if (!empty($price_range = $request->query('price_range'))) {
            ...
            $raw_sql_min_max = "( (IFNULL(bravo_saunas.sale_price,0) > 0 and bravo_saunas.sale_price >= ? ) OR (IFNULL(bravo_saunas.sale_price,0) <= 0 and bravo_saunas.price >= ? ) )
                            AND ( (IFNULL(bravo_saunas.sale_price,0) > 0 and bravo_saunas.sale_price <= ? ) OR (IFNULL(bravo_saunas.sale_price,0) <= 0 and bravo_saunas.price <= ? ) )";
            $model_sauna->WhereRaw($raw_sql_min_max,[$pri_from,$pri_from,$pri_to,$pri_to]);
        }
        ...
        if (is_array($terms) && !empty($terms)) {
            $model_sauna->join('bravo_sauna_term as tt', 'tt.target_id', "bravo_saunas.id")->whereIn('tt.term_id', $terms);
        }
        $review_scores = $request->query('review_score');
        if (is_array($review_scores) && !empty($review_scores)) {
            $where_review_score = [];
            foreach ($review_scores as $number){
                $where_review_score[] = " ( bravo_saunas.review_score >= {$number} AND bravo_saunas.review_score <= {$number}.9 ) ";
            }
            $sql_where_review_score = " ( " . implode("OR", $where_review_score) . " )  ";
            $model_sauna->WhereRaw($sql_where_review_score);
        }

        if(!empty( $service_name = $request->query("service_name") )){
            if( setting_item('site_enable_multi_lang') && setting_item('site_locale') != app()->getLocale() ){
                $model_sauna->leftJoin('bravo_sauna_translations', function ($join) {
                    $join->on('bravo_saunas.id', '=', 'bravo_sauna_translations.origin_id');
                });
                $model_sauna->where('bravo_sauna_translations.title', 'LIKE', '%' . $service_name . '%');

            }else{
                $model_sauna->where('bravo_saunas.title', 'LIKE', '%' . $service_name . '%');
            }
        }
        if(!empty($lat = $request->query('map_lat')) and !empty($lgn = $request->query('map_lgn'))){
//            ORDER BY (POW((lon-$lon),2) + POW((lat-$lat),2))";
            $model_sauna->orderByRaw("POW((bravo_saunas.map_lng-".$lgn."),2) + POW((bravo_saunas.map_lat-".$lat."),2)");
        }
        $orderby = $request->input("orderby");
        switch ($orderby){
            case "price_low_high":
                $raw_sql = "CASE WHEN IFNULL( bravo_saunas.sale_price, 0 ) > 0 THEN bravo_saunas.sale_price ELSE bravo_saunas.price END AS tmp_min_price";
                $model_sauna->selectRaw($raw_sql);
                $model_sauna->orderBy("tmp_min_price", "asc");
                break;
            case "price_high_low":
                $raw_sql = "CASE WHEN IFNULL( bravo_saunas.sale_price, 0 ) > 0 THEN bravo_saunas.sale_price ELSE bravo_saunas.price END AS tmp_min_price";
                $model_sauna->selectRaw($raw_sql);
                $model_sauna->orderBy("tmp_min_price", "desc");
                break;
            case "rate_high_low":
                $model_sauna->orderBy("review_score", "desc");
                break;
            default:
                $model_sauna->orderBy("is_featured", "desc");
                $model_sauna->orderBy("id", "desc");
        }
        
        $model_sauna->groupBy("bravo_saunas.id");

        $max_guests = (int)($request->query('adults') + $request->query('children'));
        if($max_guests){
            $model_sauna->where('max_guests','>=',$max_guests);
        }
        if(!empty($request->query('limit'))){
            $limit = $request->query('limit');
        }else{
            $limit = !empty(setting_item("sauna_page_limit_item"))? setting_item("sauna_page_limit_item") : 9;
        }
        return $model_sauna->with(['location','hasWishList','translations'])->paginate($limit);
    }
    ...
    public function dataForApi($forSingle = false){
        ...
        if($forSingle){
            ...
            $data['booking_fee'] = setting_item_array('sauna_booking_buyer_fees');
    ...
    {
        return "\Modules\Sauna\Controllers\AvailabilityController";
    }
    ...
    static public function getFiltersSearch()
    {
        ...
        return [
            ...
            [
                ...
                "field"    => "terms",
                "position" => "3",
                "data" => Attributes::getAllAttributesForApi("sauna")
            ]
        ];
    }
}
