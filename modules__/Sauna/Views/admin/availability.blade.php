...
@section ('content')
    ...
    <div class="container-fluid">
        <div class="d-flex justify-content-between mb20">
            <h1 class="title-bar">{{__("Saunas Availability Calendar")}}</h1>
        </div>
        ...
        @else
            <div class="alert alert-warning">{{__("No saunas found")}}</div>
        @endif
    ...
    <script>
        ...
        $('#items_tab').on('show.bs.tab',function (e) {
            ...
            calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                saunas:{
                    url:"{{route('sauna.admin.availability.loadDates')}}",
                    extraParams:{
                        id:lastId,
                    }
                },
                ...
                saunaClick:function (info) {
                    var form = Object.assign({},info.sauna.extendedProps);
                    form.start_date = moment(info.sauna.start).format('YYYY-MM-DD');
                    form.end_date = moment(info.sauna.start).format('YYYY-MM-DD');
                    console.log(form);
                    formModal.show(form);
                },
                saunaRender: function (info) {
                    $(info.el).find('.fc-title').html(info.sauna.title);
                    $(info.el).find('.fc-content').attr("data-html","true").attr("title",info.sauna.title).tooltip({ boundary: 'window' })
                }
            });
            calendar.render();
        });

        $('.sauna-name:first-child a').trigger('click');
        ...
        formModal = new Vue({
            ...
            methods:{
                ...
                saveForm:function () {
                    ...
                    $.ajax({
                        url:'{{route('sauna.admin.availability.store')}}',
                        data:this.form,
                        dataType:'json',
                        method:'post',
