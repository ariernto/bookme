    ...
    <div class="row">
        ...
        <div class="col-md-9">
            ...
            <div class="control-action">
                <a href="{{route('tour.vendor.clone',[$row->id])}}" target="_blank" class="btn btn-light">{{__("Clone")}}</a>
                <a href="{{$row->getDetailUrl()}}" target="_blank" class="btn btn-light">{{__("View")}}</a>
                @if(!empty($recovery))
                    <a href="{{ route("tour.vendor.restore",[$row->id]) }}" class="btn btn-recovery btn-light" data-confirm="{{__('"Do you want to recovery?"')}}">{{__("Recovery")}}</a>
                @endif
                @if(Auth::user()->hasPermissionTo('tour_update'))
                    <a href="{{ route("tour.vendor.edit",[$row->id]) }}" class="btn btn-light">{{__("Edit")}}</a>
                @endif
                @if(Auth::user()->hasPermissionTo('tour_delete'))
                    <a href="{{ route("tour.vendor.delete",[$row->id]) }}" class="btn btn-danger" data-confirm="<?php echo e(__("Do you want to delete?")); ?>">{{__("Delete")}}</a>
                @endif
                @if($row->status == 'publish')
                    <a href="{{ route("tour.vendor.bulk_edit",[$row->id,'action' => "make-hide"]) }}" class="btn btn-light">{{__("Make hide")}}</a>
                @endif
                @if($row->status == 'draft')
